...
# **\[FIX#在Chrome的**DevTools **控制台下执行** **navigator.clipboard** **返回** undefined，经查找资料发现是浏览器禁用了非安全域的navigator.clipboard对象]
...
**出错异常：无法保存图片！PHP# imagejpeg(),返回f alse 原因：目标路径不可写，或者目录不存在。**

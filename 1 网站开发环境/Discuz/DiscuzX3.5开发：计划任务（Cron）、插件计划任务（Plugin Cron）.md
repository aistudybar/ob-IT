...
if(!defined('IN_DISCUZ')) {
 ...
 global $_G;
 // 定义日志文件路径（DiscuzX3.5的日志通常放在data/log目录下）
 $log_dir = DISCUZ_ROOT . 'data/log/';
...
// 获取当前时间
// $current_time = $_G['timestamp'];
// $current_time = date('Y-m-d H:i:s'); // 格式如：2025-03-08 15:30:45
// $current_time = $_G['timenow']['time'];        //////??????有时无内容！
// 时区标识：在PHP中，'Etc/GMT+8'表示UTC-8（注意符号是反的：+表示西时区，-表示东时区）。
// 你也可以使用具体地区如'America/Los_Angeles'，但要考虑夏令时影响。
// 夏令时：如果需要处理夏令时（如美国西海岸时间），建议使用方法2并用'America/Los_Angeles'，它会自动调整。
// 如果只想要固定的UTC-8偏移，则用'Etc/GMT+8'。
$date = new DateTime('now', new DateTimeZone('America/Los_Angeles'));        // 创建DateTime对象并设置UTC-8时区
$time = $date->format('Y-m-d H:i:s');        // 格式化时间
// 准备要写入的日志内容
$log_content = "UTC-8当前时间: " . $time . "\n";
// 写入日志文件（追加模式）
file_put_contents($log_file, $log_content, FILE_APPEND | LOCK_EX);
// 可选：返回成功信息
echo "时间已记录到日志文件：$log_file";
```

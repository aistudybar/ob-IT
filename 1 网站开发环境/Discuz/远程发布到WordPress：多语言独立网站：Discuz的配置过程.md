...
-- ----------------------------
-- Table structure for pre_forum_post_lang
-- ----------------------------
DROP TABLE IF EXISTS `pre_forum_post_lang`;
CREATE TABLE `pre_forum_post_lang`(
`pid` int UNSIGNED NOT NULL,
`lang` tinytext CHARACTER SET ascii COLLATE ascii_general_ci NOT NULL,
 `lastupdate` int UNSIGNED NULL DEFAULT 0,
 ...
 `wp_post_id` int UNSIGNED NULL DEFAULT NULL,
 PRIMARY KEY (`pid` DESC, `lang`(2) DESC) USING BTREE,
 INDEX `index`(`pid` ASC) USING BTREE,
 CONSTRAINT `key` FOREIGN KEY (`pid`) REFERENCES `pre_forum_post` (`pid`) ON DELETE CASCADE ON UPDATE RESTRICT
